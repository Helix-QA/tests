...
Сценарий: Я Оплатил документ для Юкассы	
	...
	И Я нажимаю на кнопку оплаты 'КнопкаВидыОплатПрименитьеСуммуСтрока_1x1'
...
Сценарий: Я открываю обработку - ТестОплатПоЮКассеТабличка.
	Дано я открываю внешнюю обработку или отчет "$КаталогПроекта$\epf\ТестОплатПоЮКассеТабличка.epf" (Расширение)

//Cloudpayment

Сценарий: Я создаю продажу Cloudpayment.
	Дано Я открываю основную форму документа "Реализация"
	И в поле с именем 'Контрагент' я ввожу текст "$$Фамилия$$"
	И из выпадающего списка с именем 'Контрагент' я выбираю по строке "$$Фамилия$$"
	И я нажимаю на кнопку с именем 'ЗапасыКнопкаДобавить'
	И в таблице 'Запасы' в поле с именем 'ЗапасыНоменклатура' я ввожу текст "$$НоменклатураЮкассы$$"
	И в таблице 'Запасы' из выпадающего списка с именем 'ЗапасыНоменклатура' я выбираю по строке "$$НоменклатураЮкассы$$"
	И Я Оплатил документ для Cloudpayment.

Сценарий: Я создаю продажу Cloudpayment с учетом СНО.
*Определение что за система СНО.
	Если выражение внутреннего языка '"$СНО$" = "Упрощенная доход"' Истинно Тогда
		Тогда Я запоминаю значение выражения 'Формат(1, "ЧГ=0")' в переменную "НомерСНО"
	Если выражение внутреннего языка '"$СНО$" = "Упрощенная доход минус расход"' Истинно Тогда
		Тогда Я запоминаю значение выражения 'Формат(2, "ЧГ=0")' в переменную "НомерСНО"
	Если выражение внутреннего языка '"$СНО$" = "Единый налог на вмененный доход"' Истинно Тогда
		Тогда Я запоминаю значение выражения 'Строка(Формат("0", "ЧГ=0"))' в переменную "НомерСНО"
	Если выражение внутреннего языка '"$СНО$" = "Единый сельскохозяйственный налог"' Истинно Тогда
		Тогда Я запоминаю значение выражения 'Формат(1, "ЧГ=0")' в переменную "НомерСНО"
	Если выражение внутреннего языка '"$СНО$" = "Патентная система налогообложения"' Истинно Тогда
		Тогда Я запоминаю значение выражения 'Формат(3, "ЧГ=0")' в переменную "НомерСНО"
...
*Создание продажи.
	...
	И я нажимаю на кнопку с именем 'КнопкаОплатитьРеализацию'
	И я нажимаю на кнопку с именем 'КассаОплаты_1'
	И я устанавливаю флаг с именем 'ЗадолженностьВидНалогаВыборСтрока_$НомерСНО$'
...
Сценарий: Я создаю продажу ВТБ с учетом СНО.
*Определение что за система СНО.
	Если выражение внутреннего языка '"$СНО$" = "Упрощенная доход"' Истинно Тогда
		Тогда Я запоминаю значение выражения 'Формат(1, "ЧГ=0")' в переменную "НомерСНО"
	Если выражение внутреннего языка '"$СНО$" = "Упрощенная доход минус расход"' Истинно Тогда
...
Сценарий: Я Оплатил документ для Cloudpayment.
	И я нажимаю на кнопку с именем 'КнопкаОплатитьРеализацию'
	И я нажимаю на кнопку с именем 'КассаОплаты_1'
	Когда открылось окно "Оплата"
	И я снимаю флаг "ЗадолженностьУстановитьСнятьГалочки"
...
Сценарий: Я создаю продажу ВТБ.
	...
	И в таблице 'Запасы' в поле с именем 'ЗапасыНоменклатура' я ввожу текст "$$НоменклатураЮкассы$$"
	И в таблице 'Запасы' из выпадающего списка с именем 'ЗапасыНоменклатура' я выбираю по строке "$$НоменклатураЮкассы$$"
	И Я Оплатил документ для ВТБ.

Сценарий: Я Оплатил документ для ВТБ.
	И я нажимаю на кнопку с именем 'КнопкаОплатитьРеализацию'
	И я нажимаю на кнопку с именем 'КассаОплаты_1'
	Когда открылось окно "Оплата"
	И я снимаю флаг "ЗадолженностьУстановитьСнятьГалочки"
...
Сценарий: Я создаю продажу PayU.
	Дано Я открываю основную форму документа "Реализация"
	И в поле с именем 'Контрагент' я ввожу текст "$$Фамилия$$"
	И из выпадающего списка с именем 'Контрагент' я выбираю по строке "$$Фамилия$$"
	И я нажимаю на кнопку с именем 'ЗапасыКнопкаДобавить'
	И в таблице 'Запасы' в поле с именем 'ЗапасыНоменклатура' я ввожу текст "$$НоменклатураЮкассы$$"
	И в таблице 'Запасы' из выпадающего списка с именем 'ЗапасыНоменклатура' я выбираю по строке "$$НоменклатураЮкассы$$"
	И Я Оплатил документ для PayU.

Сценарий: Я Оплатил документ для PayU.
	И я нажимаю на кнопку с именем 'КнопкаОплатитьРеализацию'
	И я нажимаю на кнопку с именем 'КассаОплаты_1'
	Когда открылось окно "Оплата"
	И я снимаю флаг "ЗадолженностьУстановитьСнятьГалочки"
	И я устанавливаю флаг с именем 'ЗадолженностьВыборСтрока_0'
	Когда открылось окно "Оплата"
	И я нажимаю на кнопку с именем 'КнопкаВидыОплатаНаименованиеСтрока_1x5'
	И Я нажимаю на кнопку оплаты 'КнопкаВидыОплатПрименитьеСуммуСтрока_1x5'
	И я нажимаю на кнопку с именем 'Оплатить'
	И я нажимаю на кнопку с именем 'СформироватьСсылкуНаОплатуСВыбраннойСуммой'
