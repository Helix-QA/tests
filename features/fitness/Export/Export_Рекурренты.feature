...
Сценарий: Я создаю второе рекуррентное членство
	...
	*Настройка раздела по рекуррентам
		...
		И я запоминаю значение поля с именем 'Код' как 'КодВторогоРекуррента'
		И я закрываю все окна клиентского приложения
...
Сценарий: Я создаю третье рекуррентное членство
	...
	И я запоминаю значение поля с именем 'Код' как 'КодТретьегоРекуррента'
...
Сценарий: Я определяю даты для схемы платежей без учета заморозки.
...
И я выполняю код встроенного языка
	"""bsl
		...
		КонецСледующегоМесяца = ДобавитьМесяц(ДатаОбъекта, 2);
...
Сценарий: Я определяю даты для схемы платежей с учетом заморозки.
//Определение последней даты в текущем месяце
И я выполняю код встроенного языка
	"""bsl
		ТекущаяДатаСтрокой = Контекст.ПрошлыйМесяц;
		...
		Год = Сред(ТекущаяДатаСтрокой, 7, 4);
		ДатаОбъекта = Дата(Год, Месяц, День);
		НоваяДата = ДобавитьМесяц(ДатаОбъекта, 1)+Контекст.РазницаДней*86400;
		ТекущаяДата = Формат(НоваяДата, "ДФ=dd.MM.yyyy");
		Контекст.Вставить("КонецТекущегоМесяца", ТекущаяДата);
	"""

И Я запоминаю значение выражения '' в переменную "КонецСледующегоМесяца"
//И я выполняю код встроенного языка
//	"""bsl
//		ТекущаяДатаСтрокой=Контекст.КонецСледующегоМесяца;
//		ТекущаяДатаСтрокой=Формат(КонецМесяца(ДобавитьМесяц(Контекст.КонецТекущегоМесяца, 1)), "ДФ=дд.ММ.гггг");
//		Контекст.Вставить("КонецСледующегоМесяца", ТекущаяДатаСтрокой);
//	"""
...
И я выполняю код встроенного языка
	"""bsl
		...
		НоваяДата = ДобавитьМесяц(ДатаОбъекта, 1);
		ТекущаяДата = Формат(НоваяДата, "ДФ=dd.MM.yyyy");
		Контекст.Вставить("КонецСледующегоМесяца", ТекущаяДата);
	"""
