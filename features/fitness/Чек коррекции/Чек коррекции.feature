...
Сценарий: ЧекКоррекции_1 (Поступление)
	...
	*Проверка сообщения в "ЧекКоррекции"
		...
		Тогда в логе сообщений TestClient есть строки по шаблону:
			|'По данному документу ранее была создана корректировка документом Чек коррекции 36 от $$ЦифроваяДата$$ * . \nСоздание повторной корректировки не возможно.'|
		И я закрываю текущее окно
...
Сценарий: ЧекКоррекции_2 (Списание)
	*Переход в "Создание возврата"
		...
		И я нажимаю на кнопку с именем 'КнопкаВидыОплатПрименитьеСуммуСтрока_0'
		И я нажимаю на кнопку с именем 'Оплатить'
		И я закрываю все окнка клиентского приложения	
	...
	*Проверка "Исходные данные чека"
		Тогда элемент формы с именем 'XML' стал равен 
			...
			|'		<CorrectionData Type=\"0\" Date=\"$$ЦифроваяДата$$T00:00:00\" Description=\"Ошибка в указании ставки НДС при применении ККТ\"/>'|
			|'		<AgentData/>'|
			|'		<VendorData/>'|
			|'		<CustomerDetail/>'|
			|'		<OperationalAttribute/>'|
