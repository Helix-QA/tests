...
Сценарий: 01. Создание условий для проверки	
	...
	И в таблице "Список" я перехожу к строке по шаблону:
		| "Как клиент"                                 | "ФИО"                            |
		| "<b><colorstyle -46>Админ</></> (Сотрудник)" | "<b><colorstyle -46>Админ</></>" |
	И в таблице 'Список' я выбираю текущую строку
...
/////МЕТОД "comments" выполняется с ошибкой - ТИКЕТ 000667510.
	...
	И из выпадающего списка с именем 'Метод' я выбираю точное значение "possible_durations"
	И я нажимаю на кнопку с именем 'ВыполнитьБезHTTPСоединения'
	Если таблица "РезультатДерево" не содержит строки по шаблону: Тогда
		| 'Ключ'     | 'Значение' |
		| 'duration' | '1 час*'   |
		Тогда таблица "РезультатДерево" содержит строки по шаблону:
			| 'Ключ'     | 'Значение' |
...
Сценарий: Проверка метода cart_cost с промокодом
*Удаление данных и создание нового клиента.
	И я удаляю все переменные
	И Я создаю клиента
	И Я создаю Услуга_Персональная
	И Я создаю Тренера

*Включение тумблера "Онлайн продажа" в персональной услуге
	Дано Я открываю навигационную ссылку "$$СсылкаУслугаПерсональная$$"
	И я устанавливаю флаг с именем 'ОнлайнПродажа'
	И Я запоминаю случайное число в переменную "ОнлайнГруппа"
	Дано я запоминаю значение выражения '"ОнлайнГруппа" + Формат($ОнлайнГруппа$, "ЧГ=0")' в переменную "ОнлайнГруппа"	
	И я нажимаю на кнопку создать поля с именем 'ГруппаОнлайнПродажи'
	Тогда открылось окно "Онлайн-каталог (создание)"
	И в поле с именем 'Наименование' я ввожу текст "$ОнлайнГруппа$"
	И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'
	И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'

*Добавление почты в карточку клиента
...
*Создание промокода.
	...
	Тогда открылось окно 'Вид промокода'
...
*Создание маркетинговой акции по промокоду.
	...
	И в таблице "Список" я перехожу к строке:
		...
		| 'Скидки'              |
	И я нажимаю на кнопку с именем "Создать"
	Тогда открылось окно 'Скидка: создание элемента'
	...
	И в поле с именем 'ЗначениеСкидкиНаценки' я ввожу текст "5,00"
	И в поле с именем "Наименование" я ввожу текст "$$Промо$$"
	И из выпадающего списка с именем 'ВидПромокода' я выбираю по строке "$$Промо$$"
	И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'					

*Открытие обработки и выбор метода "cart_cost" с дальнейшим его заполнением.
	И Я открываю обработку - ТестАпиОбщую.
	Когда открылось окно "Api test"
	И я нажимаю на кнопку с именем 'Api_v3'
	И я нажимаю на кнопку с именем 'ТипЗапроса'
	И я выбираю из списка 'GET'
	И из выпадающего списка с именем 'Метод' я выбираю по строке "cart_cost"
	И я нажимаю кнопку выбора у поля с именем 'ВнешняяСистема'
	Тогда открылось окно "Внешние системы"
	И в таблице 'Список' я перехожу к строке по шаблону:
		| "Наименование"        |
		| "Виджет расписания-*" |
	И в таблице "Список" я выбираю текущую строку			
	И из выпадающего списка с именем 'СтруктурнаяЕдиница' я выбираю точное значение "Фитнес плюс"
	И в поле с именем 'client' я ввожу текст "$$Клиент$$"
	И из выпадающего списка с именем 'client' я выбираю "$$Клиент$$"
	И в поле с именем 'promocode' я ввожу текст "$$Промокод$$"
	И я выбираю пункт контекстного меню с именем 'Таблица_cartКонтекстноеМенюДобавить' на элементе формы с именем 'Таблица_cart'
	И в таблице 'Таблица_cart' я нажимаю кнопку выбора у реквизита с именем 'Таблица_cartpurchase'
...
*Проверка метода cart_cost с промокодом					
	И таблица 'РезультатДерево' стала равной по шаблону:
		...
		| 'title'                           | 'Розничная цена'              |
		| 'price'                           | '250'                         |
		| 'price_with_discount'             | '237,5'                       |
		| 'discount_sum'                    | '12,5'                        |
		| 'payment_amount'                  | '237,5'                       |
		| 'tax'                             | ''                            |
		| 'vat'                             | '0'                           |
		| 'tax_sum'                         | '0'                           |
		| 'employee'                        | ''                            |
		...
		| 'payment_object'                  | ''                            |
		| 'payment_method'                  | ''                            |
		| 'employee_choice_required'        | 'Нет'                         |
		| 'course_choice_required'          | 'Нет'                         |
		| 'cycle_periods_choice_required'   | 'Нет'                         |
		| 'total_amount'                    | '237,5'                       |
		| 'total_discount'                  | '12,5'                        |
		| 'total_deposit'                   | '0'                           |
		| 'may_be_payment'                  | ''                            |
		...
		| 'club_id'                         | 'f*'                          |
		| 'bank_tax'                        | 'osn'                         |
		| 'use_sberbank_fiscalization'      | 'Нет'                         |
		| 'use_cloudpayments_fiscalization' | 'Нет'                         |
		| 'use_yandex_fiscalization'        | 'Да'                          |
		| 'delete_cart_bank'                | 'Да'                          |
		| 'show_gift_to'                    | 'Нет'                         |
		| 'promocode'                       | '$$Промокод$$'                |
		| 'employee_choice_required'        | 'Нет'                         |
