...
//Смена данных.
	...
	Дано Я запоминаю значение выражения 'Формат(ТекущаяДата()+6200, "ДФ=\'Ч:мм\'")' в переменную "$$СегодняДляЗанятия2$$"
...
//Проверка списания.
	...
	И таблица 'Список' содержит строки по шаблону:
		...
		| "Аренда зала: * $$СегодняДляЗанятия1$$, $$УслугаАрендаЗала2$$, Фитнес плюс, *" | "Запись на занятие * от * $$СегодняДляЗанятия2$$:00" | "1 000,00" | "$$УслугаАрендаЗала2$$, $$УслугаАрендаЗала2$$, Розничная цена" |
...
//Проверка отмены.
	...
	И элемент формы с именем 'СтатусПрибытия' стал равен "Отменил"
//Отказ от клуба.
	И я закрываю все окна клиентского приложения
