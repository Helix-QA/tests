...
Сценарий: 08. Проверка привязки платежной карты с указанием почты в СЕ - 000876388
*Открываем СЕ и добавляем почту.
	И я закрываю все окна клиентского приложения
	...
	И я нажимаю кнопку очистить у поля с именем 'ПредставлениеКИ_1'
	И в поле с именем 'ПредставлениеКИ_1' я ввожу текст "test@mail.ru"
...
*Обычная привязка карты через браузер.
	Дано Я открываю навигационную ссылку "$ТекКлиент$"
	И я нажимаю на кнопку с именем 'ДобавитьПлатежнуюКарту'
	...
	# Кликаем на способ оплаты "Кошелёк"
	И в странице браузера для элемента с именем класса "MuiTypography-root MuiTypography-bodyM css-1artrme" я делаю клик
	# Кликаем на способ оплаты "Заплатить 1"
	И в странице браузера для элемента с именем класса "MuiButtonBase-root MuiButton-root MuiButton-contained MuiButton-containedActive MuiButton-sizeLarge MuiButton-containedSizeLarge MuiButton-colorActive MuiButton-disableElevation MuiButton-fullWidth MuiButton-root MuiButton-contained MuiButton-containedActive MuiButton-sizeLarge MuiButton-containedSizeLarge MuiButton-colorActive MuiButton-disableElevation MuiButton-fullWidth css-127uj5a" я делаю клик
	...
	*Проверка статуса оплаты.
		Когда открылось окно "Привязка банковской карты в ЮКасса"
	...
	*Переоткрытие карточки клиента и проверка появления платежной карты на форме.
		И я закрываю все окна клиентского приложения
		Дано Я открываю навигационную ссылку "$ТекКлиент$"
		И элемент формы с именем 'ПлатежныеКартыСуществующие_0' стал равен по шаблону '**** **** **** **** привязка * *:*'