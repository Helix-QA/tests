	...
	*СОЗДАНИЕ ПРОДАЖИ ОСНОВОГО РЕКУРРЕНТНОГО ЧЛЕНСТВА
		...
		*Проверка добавления вступительного взноса в ТЧ номенклатуры
			И таблица 'Запасы' стала равной по шаблону:
				...
				| '2' | '1,000'  | '$$ВступительныйВзнос$$' | 'Админ'    | ''               | ''            | ''               | '2 000,00' | '2 000' | ''         | ''       | 'Админ'       | ''         | 'Розничная цена' | ''      |
...
//		Если элемент формы с именем 'СтатусОтправкиВедомости' стал равен "Ожидает отправки" Тогда
		Если представление элемента с именем 'СтатусОтправкиВедомости' стало равно 'Ожидание отправки' Тогда
...
Сценарий: Проверка функционала с определением схемы рек.платежей по дате окончания членства - 643250.
	И я удаляю все переменные
*Создание клиента и платежной связки
	И Я создаю клиента для Юкассы
	И Я запоминаю в переменную "$$СсылкаКлиентЮкасса$$" значение "$ТекКлиент$"
	...
	"""bsl
		...
		// Базовая дата — 28-е число прошлого месяца
		ДатаПрошлый = НачалоПрошлогоМесяца + 27 * 86400;

		// Проверяем, февраль ли прошлый месяц
		ЭтоФевраль = (Месяц(ДатаПрошлый) = 2);

		// Вычисляем дату следующего месяца
		Если ЭтоФевраль Тогда
			// Особое правило: после февраля → конец следующего месяца
			НачалоСледующегоМесяца = ДобавитьМесяц(НачалоПрошлогоМесяца, 1);
			ДатаСледующий = КонецМесяца(НачалоСледующегоМесяца);
		Иначе
			// Обычная логика: +1 месяц к 28-му числу
			ДатаСледующий = ДобавитьМесяц(ДатаПрошлый, 1);
		КонецЕсли;

		// Вычисляем конец прошлого месяца (для корректировки)
		КонецПрошлогоМесяца = КонецМесяца(ДатаПрошлый);
		ДеньКонца = День(КонецПрошлогоМесяца);

		// Корректируем дату (логика осталась прежней)
		...
		Иначе
			...
			// Если хотите всегда 28 февраля — раскомментируйте:
			// КорректированнаяДата = Дата(Год(КонецПрошлогоМесяца), 2, 28);
		КонецЕсли;

		// Разница в днях (между 28-м и скорректированной датой прошлого месяца)
		РазницаВСекундах = КонецМесяца(ДатаПрошлый) - ДатаПрошлый;
		ДатаЗаморозкиДо = Формат(КонецМесяца(ДатаПрошлый),"ДФ=дд.ММ.гггг");
		...
		Контекст.Вставить("РазницаДней",        РазницаДней);	
		Контекст.Вставить("ДатаЗаморозкиДо",        ДатаЗаморозкиДо);	
	"""
...
Если выражение внутреннего языка 'Контекст.РазницаДней > 0' Истинно Тогда
...
//		И в таблице "Список" я выбираю текущую строку
	...
	*Определение дат без учета заморозки вступительного членства.	
		И Я определяю даты для схемы платежей без учета заморозки.
		И таблица 'ТаблицаСхемыПлатежей' содержит строки по шаблону:
			| 'Номер'         | 'Дата'                    | 'Членство'      | 'Операция с членством'                       | 'Членство на будущий период' | 'Оплата'                                              | 'Сумма оплаты' | 'Статус оплаты'                          |
			| 'Вступительное' | '$ПрошлыйМесяц$'          | '$$Рекуррент$$' | ''                                           | '$$Рекуррент$$'              | 'Оплачено ({Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")})' | '3 500,00'     | 'Подтвержден банком или оплачен вручную' |
	...
	*Открытие договора рек.платежей и проверка дат в нем
		...
		И в таблице 'Список' я перехожу к строке по шаблону:
			...
			| 'Рек. договор: * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}, $$Фамилия$$' | ''                 | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}' | ''       | '$ПрошлыйМесяц$' | 'Нет'     | 'Основная ораганизация' | 'Открыт'          | '$$Фамилия$$' | 'Фитнес плюс'         | '\'SRP: ЮКасса\''  |
		И в таблице "Список" я выбираю текущую строку
		И Я определяю даты для схемы платежей с учетом заморозки.	
		И Я запоминаю значение выражения 'Формат($РазницаДней$, "ЧГ=0")' в переменную "РазницаДней"
		И таблица 'ТаблицаСхемыПлатежей' стала равной по шаблону:
			| 'Номер'         | 'Дата'                    | 'Членство'      | 'Членство на будущий период' | 'Операция с членством'                                                 | 'Оплата'                                              | 'Сумма оплаты' | 'Статус оплаты'                          |
			| 'Вступительное' | '$ПрошлыйМесяц$'          | '$$Рекуррент$$' | '$$Рекуррент$$'              | ''                                                                     | 'Оплачено ({Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")})' | '3 500,00'     | 'Подтвержден банком или оплачен вручную' |
			| ''              | '$ПрошлыйМесяц$'          | ''              | ''                           | 'Заморозка с $ПрошлыйМесяц$ по $ДатаЗаморозкиДо$ на $РазницаДней$ дня' | ''                                                    | ''             | ''                                       |
			| '1'             | '$КонецТекущегоМесяца$'   | '$$Рекуррент$$' | '$$Рекуррент$$'              | ''                                                                     | 'Оплачено ({Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")})' | '1 500,00'     | 'Подтвержден банком или оплачен вручную' |
			| '2'             | '$КонецСледующегоМесяца$' | ''              | '$$Рекуррент$$'              | ''                                                                     | 'Оплатить вручную'                                    | '1 500,00'     | ''                                       |
...
Сценарий: Проверка настройки стратегии списания - 000763082.
	И я удаляю все переменные
*Создание клиента и платежной связки
	И Я создаю клиента для Юкассы
	И Я запоминаю в переменную "$$СсылкаКлиентЮкасса$$" значение "$ТекКлиент$"
...
*Создание продажи первого рекуррентного членства.
	Дано я открываю	основную форму документа "Реализация"
	И в поле с именем 'Контрагент' я ввожу текст "$$Фамилия$$"
	И из выпадающего списка с именем 'Контрагент' я выбираю "$$Фамилия$$"
...
*Проверка возможности выбора второго и третьего созданного рекуррентного членства.
	*Проверка выбора второго рекуррента.
		И в таблице 'ТаблицаСхемыПлатежей' я перехожу к строке:
			| "Номер" | "Членство на будущий период" |
			| "1"     | "$$Рекуррент$$"              |
		И в таблице 'ТаблицаСхемыПлатежей' я активизирую поле с именем 'ТаблицаСхемыПлатежейНоменклатураРеализуемогоЧленства'
		И в таблице 'ТаблицаСхемыПлатежей' я выбираю текущую строку
		И в таблице 'ТаблицаСхемыПлатежей' в поле с именем 'ТаблицаСхемыПлатежейНоменклатураРеализуемогоЧленства' я ввожу текст "$$ВторойРекуррент$$"
		И в таблице "ТаблицаСхемыПлатежей" выпадающий список с именем 'ТаблицаСхемыПлатежейНоменклатураРеализуемогоЧленства' стал равен:
			| '$$ВторойРекуррент$$ ($КодВторогоРекуррента$)' |
		И в таблице 'ТаблицаСхемыПлатежей' я нажимаю кнопку выбора у реквизита с именем 'ТаблицаСхемыПлатежейНоменклатураРеализуемогоЧленства'
		И я активизирую дополнение формы с именем 'СтрокаПоиска'
		И в дополнение формы с именем 'СтрокаПоиска' я ввожу текст "$$ВторойРекуррент$$"
		И я жду, что в таблице "Список" количество строк будет "больше" 0 в течении 20 секунд
		И в таблице 'Список' я перехожу к строке по шаблону:
			| 'Наименование'          |
			| '*$$ВторойРекуррент$$*' |
		И в таблице 'Список' я выбираю текущую строку 		
	*Проверка выбора третьего рекуррента.
		И в таблице 'ТаблицаСхемыПлатежей' я активизирую поле с именем 'ТаблицаСхемыПлатежейНоменклатураРеализуемогоЧленства'
		И в таблице 'ТаблицаСхемыПлатежей' я выбираю текущую строку
		Когда открылось окно "Рек. договор: * от * (Активен) *"
		И в таблице 'ТаблицаСхемыПлатежей' я нажимаю кнопку выбора у реквизита с именем 'ТаблицаСхемыПлатежейНоменклатураРеализуемогоЧленства'
		...
		И я активизирую дополнение формы с именем 'СтрокаПоиска'
		И в дополнение формы с именем 'СтрокаПоиска' я ввожу текст "$$ТретийРекуррент$$"
		И я жду, что в таблице "Список" количество строк будет "равно" 0 в течении 20 секунд
		И в таблице "Список" количество строк 'равно' 0		
		И я закрываю все окна клиентского приложения
		
*Проверка настройки схемы платежей в номенклатуре
	Дано Я открываю навигационную ссылку "$$СсылкаОсновнойРекуррент$$"
	Когда открылось окно "Членство - $$Рекуррент$$"
	*Проверка со вторым рекуррентным членством.
		И я нажимаю на кнопку с именем 'КнопкаДобавитьАвтопролонгация'
		И в таблице 'ЧленствоПакетУслугРекуррентностьСтратегия' я активизирую поле с именем 'ЧленствоПакетУслугРекуррентностьСтратегия1НоменклатураЧленства'
		И в таблице 'ЧленствоПакетУслугРекуррентностьСтратегия' в поле с именем 'ЧленствоПакетУслугРекуррентностьСтратегия1НоменклатураЧленства' я ввожу текст "$$ВторойРекуррент$$"
		И в таблице "ЧленствоПакетУслугРекуррентностьСтратегия" выпадающий список с именем 'ЧленствоПакетУслугРекуррентностьСтратегия1НоменклатураЧленства' стал равен:
				| '$$ВторойРекуррент$$ ($КодВторогоРекуррента$)' |
		И в таблице 'ЧленствоПакетУслугРекуррентностьСтратегия' из выпадающего списка с именем 'ЧленствоПакетУслугРекуррентностьСтратегия1НоменклатураЧленства' я выбираю точное значение "$$ВторойРекуррент$$ ($КодВторогоРекуррента$)"
		И в таблице 'ЧленствоПакетУслугРекуррентностьСтратегия' я нажимаю кнопку выбора у реквизита с именем 'ЧленствоПакетУслугРекуррентностьСтратегия1НоменклатураЧленства'
		Тогда открылось окно "Номенклатура"
		И я активизирую дополнение формы с именем 'СтрокаПоиска'
		И в дополнение формы с именем 'СтрокаПоиска' я ввожу текст "$$ВторойРекуррент$$"
		И я жду, что в таблице "Список" количество строк будет "больше" 0 в течении 20 секунд
		И в таблице 'Список' я перехожу к строке по шаблону:
			...
			| '*$$ВторойРекуррент$$*' |
		И в таблице 'Список' я выбираю текущую строку 		
	*Проверка со третьим рекуррентным членством.		
		И в таблице 'ЧленствоПакетУслугРекуррентностьСтратегия' я нажимаю кнопку выбора у реквизита с именем 'ЧленствоПакетУслугРекуррентностьСтратегия1НоменклатураЧленства'
		Тогда открылось окно "Номенклатура"
		И я активизирую дополнение формы с именем 'СтрокаПоиска'
		И в дополнение формы с именем 'СтрокаПоиска' я ввожу текст "$$ТретийРекуррент$$"
