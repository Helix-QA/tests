...
//	И я удаляю объекты "Справочники.Номенклатура" без контроля ссылок
	...
	*Открытие загруженного Пакета услуг
		...
		*Включение настройки "Выбирать вид цен"
			...
			И я нажимаю кнопку очистить у поля с именем 'ВидЛицевогоСчетаЧленстваПакетыУслуг'
			И я нажимаю кнопку очистить у поля с именем 'ЗачислитьНаЛицевойСчетЧленстваПакетыУслуг'
...
Сценарий: Проверка настройки "Резервирование при продаже:" в пакете услуг
	...
	*Создание продажи пакета услуг
		...
		И я нажимаю на кнопку 'Тестовый тренер'
		И я нажимаю на кнопку с именем 'ОткрытьФормуПланирования'
		Тогда элемент формы с именем 'Сотрудник' стал равен "$$НаименованиеТренера1$$"
	...
	*Проверка что занятие создалось с помощью резерва.
...
Сценарий: Проверка настройки "Невозможно использовать без активного членства" в пакете услуг
	...
	*Создание продажи членства
		...
		И в таблице '_ПодборНоменклатур' я перехожу к строке:
			| "Номенклатура" |
			| "Членства"     |
		И в таблице '_ПодборНоменклатур' я перехожу на один уровень вниз
		И в таблице '_ПодборНоменклатур' я перехожу к строке:
			| "Номенклатура"             |
...
//		И я нажимаю на кнопку с именем 'ЗапасыКнопкаДобавить'
//		И в таблице 'Запасы' я выбираю текущую строку
//		И в таблице 'Запасы' в поле с именем 'ЗапасыНоменклатура' я ввожу текст "$$НаименованиеЧленства$$"
...
@ПакетПосещений
Сценарий: Проверка настройки "Старт времени: При проходе через турникет" в пакете посещений.
	...
	И в таблице 'ДеревоОбъектов' я разворачиваю текущую строку
	...
	И я активизирую дополнение формы с именем 'СтрокаПоиска'
	И в дополнение формы с именем 'СтрокаПоиска' я ввожу текст "Пакет посещений"
	Если в таблице "Список" количество строк "больше" 0 Тогда
		Тогда в таблице "Список" я перехожу к строке по шаблону:
			...
			| '<b><colorstyle -46>Пакет</></> <b><colorstyle -46>посещений</></>' | '<b><colorstyle -46>Пакет</></> услуг' |		
		И в таблице "Список" я выбираю текущую строку						
		И я нажимаю на кнопку с именем 'ФормаУдалитьОбъект'
		Тогда открылось окно "Удалить навсегда?"
		И я нажимаю на кнопку с именем 'Button0'
	И я закрываю все окна клиентского приложения
	...
	*Отключение настройки "Использовать ключи" в рецепции
		Дано я открываю основную форму обработки 'РабочийСтол'
...
//		И в таблице 'Запасы' из выпадающего списка с именем 'ЗапасыНоменклатура' я выбираю "$$НаименованиеПакетаПосещений$$"
	...
	*Проверка списания посещений при входе с рецепции
		...
		Если элемент формы с именем 'ТекущийКлиент' стал равен "" Тогда
			И из выпадающего списка с именем 'ТекущийКлиент' я выбираю "$$Клиент$$"				
		И таблица 'ДоступныеУслугиКлиента' содержит строки по шаблону:
			| 'Группировка предст' | 'Дополнительное действие с чпу' | 'Количество предст' | 'Действие предст' |
			| 'Пакеты услуг'       | ''                              | 'остаток (резерв)'  | ''                |
...
@ПакетПосещений
Сценарий: Проверка настройки "Старт времени: При проходе через турникет" количество посещений определяется "При продаже" - 805291.
	И я удаляю объекты "Документы.Посещение" без контроля ссылок
	И Я удаляю пакет посещений.
	И я удаляю все переменные
	
	*Загрузка данных
		И Я загружаю пакет посещений 'Пакет посещений' '2600'
		И Я создаю клиента
	*Смена настройки определение кол-ва посещений "При продаже"
		...
		И я нажимаю на кнопку с именем 'УдалитьСтрочкуУслугСтрока_1'
		И пауза 2				
		И В открытой форме я открываю выпадающий список с именем 'ПакетПосещенийТипОпределенияКоличества'
		И из выпадающего списка с именем 'ПакетПосещенийТипОпределенияКоличества' я выбираю "определяется при продаже"
		И из выпадающего списка с именем 'ПакетПосещенийВидЦенОпределяемомПриПродаже' я выбираю "Розничная цена"
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
	*Создание продажи пакета посещений.
		...
		И я нажимаю на кнопку с именем 'ЗапасыКнопкаДобавить'
		И в таблице 'Запасы' я выбираю текущую строку
		И в таблице 'Запасы' в поле с именем 'ЗапасыНоменклатура' я ввожу текст "Пакет посещений"
		И в таблице 'Запасы' из выпадающего списка с именем 'ЗапасыНоменклатура' я выбираю "Пакет посещений"
		И я активизирую окно 'Ввод количества услуг'						
		И в поле с именем 'ПолеКоличествоПосещений' я ввожу текст "10"
		И я перехожу к следующему реквизиту
		И я нажимаю на кнопку с именем 'КнопкаСохранитьИзменения'
		И В таблице "Запасы" я завершаю редактирование строки
		И Я Оплатил документ 
	...
	*Проверка списания посещений при входе с рецепции
		...
		И таблица 'ДоступныеУслугиКлиента' содержит строки по шаблону:
			...
			| 'Пакеты услуг'       | ''                              | 'остаток (резерв)'                  | ''                |
			| 'Пакет посещений'    | 'Активен по $КонецЧленства$'    | '{Контекст.ОстатокПосещений} пос. ' | ''                |
		//Вычисляем остаток посещений и их сумму списания
		И Я запоминаю значение выражения 'Формат(Число(10)-Число($ОстатокПосещений$), "ЧГ=0")' в переменную "ВсегоПосещений"
		И Я запоминаю значение выражения 'Формат(Число($ВсегоПосещений$)*Число(2600), "ЧДЦ=2")' в переменную "СуммаЗаПосещения"
		И в таблице 'ДоступныеУслугиКлиента' я перехожу к первой строке
		И в таблице 'ДоступныеУслугиКлиента' я перехожу на одну строку вниз
		И в таблице 'ДоступныеУслугиКлиента' я выбираю текущую строку
		Тогда открылось окно 'Пакет услуг "Пакет посещений" * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:*'									
		И В текущем окне я нажимаю кнопку командного интерфейса "История списаний"
		И таблица 'ИсторияВзаимодействия' содержит строки по шаблону:
			...
			| '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:*:*' | 'Посещение * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:* ' | ''                                 | '-$ВсегоПосещений$,000' | ''       | ''     | ''           | ''          | '-$СуммаЗаПосещения$' |

@ПакетПосещений
Сценарий: Проверка настройки "Старт времени: При выдаче ключа" в пакете посещений.
//	И я удаляю объекты "Справочники.Номенклатура" без контроля ссылок
	И я удаляю объекты "Документы.Посещение" без контроля ссылок
	И Я удаляю пакет посещений.
	И я удаляю все переменные
	...
	*Включение соответствующей настройки
		Дано Я открываю навигационную ссылку "e1cib/data/Справочник.Номенклатура?ref=849ee6059d08fa4d11f08d75e40523a0"
		И из выпадающего списка с именем 'ПакетПосещенийСтартВремени' я выбираю "При выдаче ключа"
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
	...
	*Выдача ключа клиенту
		...
		*Проверка списания посещений при входе с рецепции
			...
			И из выпадающего списка с именем 'ТекущийКлиент' я выбираю "$$Клиент$$"				
			Если таблица 'ДоступныеУслугиКлиента' не содержит строки по шаблону: Тогда
				| 'Группировка предст' | 'Дополнительное действие с чпу' | 'Количество предст' | 'Действие предст' |
				| 'Пакеты услуг'       | ''                              | 'остаток (резерв)'  | ''                |
				| 'Пакет посещений'    | 'Активен по *'                  | '9 пос. '           | ''                |
				Тогда Я повторно создаю посещение для сценария "Проверка настройки \"Старт времени: При выдаче ключа\" в пакете посещений.".
			Иначе пауза 0
			И в таблице 'ПосещенияНаСегодня' я активизирую поле с именем 'ПосещенияНаСегодняПредставление'
			И в таблице 'ПосещенияНаСегодня' я выбираю текущую строку
			И Я запоминаю значение выражения '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}' в переменную "ТекущаяДата"
...
@ПакетПосещений
Сценарий: Проверка настройки "Ограничить доступ без резерва" в пакете посещений.
	И я удаляю объекты "Документы.Посещение" без контроля ссылок
	И Я удаляю пакет посещений.
	И я удаляю все переменные
...
*Запись на персональное занятие
	...
	И в поле с именем 'КоличествоОказываемыхУслуг' я ввожу текст "9,000"
	И я перехожу к следующему реквизиту
	И я меняю значение переключателя с именем 'СтатусЗанятия' на "Выполнено"
	И я нажимаю на кнопку с именем 'ФормаКнопкаПровестиИЗакрыть'
	Если открылось окно "1С:Предприятие" Тогда
...
*Проверка суммы списания по персональному занятию
	...
	И таблица 'ИсторияВзаимодействия' содержит строки по шаблону:
		...
		| '$ТекущаяДата$ *:*:*'   | 'Персональное: $ТекущаяДата$ *:*, Персональная услуга, Фитнес плюс, * (выполнено)' | ''                                 | '-11 250,000' | ''        | ''                 | ''           | ''          | '-11 250,00' |
...
*Проверка остатка суммы по пакету после персонального занятия
	...
	И таблица 'ДоступныеУслугиКлиента' стала равной по шаблону:
		| 'Группировка предст'              | 'Дополнительное действие с чпу' | 'Количество предст' | 'Действие предст' |
		| 'Пакеты услуг'                    | ''                              | 'остаток (резерв)'  | ''                |
		| '$$НаименованиеСуммовогоПакета$$' | 'Активен по *.*.*'              | '38 750 ₽'          | ''                |
		| '*'                               | ''                              | 'неогр.'            | 'Использовать'    |
		| '*'                               | ''                              | 'неогр.'            | 'Использовать'    |
...
*Запись на групповое занятие
	...
	И в поле с именем 'ПолеКоличества_0' я ввожу текст "25,000"
	И я перехожу к следующему реквизиту
	И я нажимаю на кнопку с именем 'КнопкаСтатусаПрибытия_0'
	И я меняю значение переключателя с именем 'СтатусЗанятия' на "Выполнено"
	И я нажимаю на кнопку с именем 'ФормаКнопкаПровестиИЗакрыть'
...
*Проверка суммы списания по групповому занятию
	...
	И таблица 'ИсторияВзаимодействия' содержит строки по шаблону:
		...
		| '$ТекущаяДата$ 0:00:00' | 'Пакет услуг "Пакет суммовой" * от $ТекущаяДата$ *:* '                             | ''                                 | ''            | 'Активен' | ''                 | ''           | ''          | ''           |
		| '$ТекущаяДата$ *:*:*'   | 'Персональное: $ТекущаяДата$ *:*, Персональная услуга, Фитнес плюс, * (выполнено)' | ''                                 | '-11 250,000' | ''        | ''                 | ''           | ''          | '-11 250,00' |
		| '$ТекущаяДата$ *:*:*'   | 'Групповое: $ТекущаяДата$ *:*, Групповая услуга, Фитнес плюс, * (выполнено)'       | ''                                 | '-38 750,000' | ''        | ''                 | ''           | ''          | '-38 750,00' |
