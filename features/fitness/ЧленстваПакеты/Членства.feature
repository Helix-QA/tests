...
//Добавление заморозок в Членство.
	...
	И я жду закрытия окна "Изменение условий членства, пакета услуг (создание) *" в течение 5 секунд
...
//Проверяю на кол-во дней которые были использованы.	
	И таблица "Список"	содержит строки по шаблону:
		| 'Период'          | 'N' | 'Основание'                                          | 'Регистратор'                                              | 'Дней заморозок' | 'Структурная единица' |
		| '$Вчера$ 0:00:00' | '2' | 'Членство "$$ЧленствоЧПУ$$" * от $ТекущаяДата$ *:* ' | 'Операция с членством (заморозка) * от $ТекущаяДата$ *:* ' | '2'              | 'Фитнес плюс'         |
//Проверяю кол-во дней которые добавились.Должен быть 1.
	И таблица "Список" содержит строки по шаблону:
		| 'N' | 'Основание'                                          | 'Регистратор'                                              | 'Дней дополнительно' | 'Структурная единица' |
		| '1' | 'Членство "$$ЧленствоЧПУ$$" * от $ТекущаяДата$ *:* ' | 'Операция с членством (заморозка) * от $ТекущаяДата$ *:* ' | '2'                  | 'Фитнес плюс'         |
...
Сценарий: Проверка возможности использовать неограниченное кол-во услуг с рецепции.//Тикет - 828410
*Удаление переменных и создание новых данных.	
	И я удаляю все переменные
	И я создаю клиента
	И Я создаю членство_ЧПУ для тикета - 828410.
	И Я создаю Помещение
	И Я тренера для проверки занятий.
*Создание продажи нового членства.
	И Я создаю продажу членства_ЧПУ

*Открытие рецепции и проверка на наличие кнопки "Использовать".
	Дано Я открываю основную форму обработки "РабочийСтол"
	...
	И таблица 'ДоступныеУслугиКлиента' содержит строки по шаблону:
		...
		| 'Членства'               | ''                              | 'остаток (резерв)'  | ''                |
		| '$$ЧленствоЧПУ$$'        | 'Активен по *.*.*'              | ''                  | ''                |
		| '$$УслугаПерсональная$$' | ''                              | 'неогр.'            | 'Использовать'    |
		| '$$УслугаГрупповая$$'    | ''                              | 'неогр.'            | 'Использовать'    |
*Запись на персональное занятие через кнопку "Использовать".
	И в таблице 'ДоступныеУслугиКлиента' я перехожу к строке:
		| "Группировка предст"     | "Действие предст" | "Количество предст" |
		| "$$УслугаПерсональная$$" | "Использовать"    | "неогр."            |
	И в таблице 'ДоступныеУслугиКлиента' я активизирую поле с именем 'ДоступныеУслугиДействие'	
	И в таблице 'ДоступныеУслугиКлиента' я выбираю текущую строку
	И я жду открытия окна "Запись на персональное занятие" в течение 5 секунд
	Когда открылось окно "Запись на персональное занятие"
	И в поле с именем 'ОтборСотрудник' я ввожу текст "$$Тренер$$"
	И из выпадающего списка с именем 'ОтборСотрудник' я выбираю "$$Тренер$$"
	И я нажимаю на гиперссылку с именем 'ДекорацияВесьДень'	
	И клик по координатам "418" "300"			
	Тогда элемент формы с именем 'Номенклатура' стал равен "$$УслугаПерсональная$$"
	...
	И в поле с именем 'ВремяНачала' я ввожу текст "00:00"		
	И я меняю значение переключателя с именем 'СтатусЗанятия' на "Выполнено"
	И я нажимаю на кнопку с именем 'ФормаКнопкаПровестиИЗакрыть'
	*Проверка списания услуги из членства по регистру ОказаниеУслуг.
		Дано Я открываю основную форму списка регистра накопления "ОказаниеУслуг"
		Когда открылось окно "Оказание услуг"
		...
		И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' в поле с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборЛевоеЗначение' я ввожу текст "Документ оплаты.Номенклатура"
		И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' из выпадающего списка с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборЛевоеЗначение' я выбираю по строке "Документ оплаты.Номенклатура"
		...
		И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' в поле с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборПравоеЗначение' я ввожу текст "$$ЧленствоЧПУ$$"
		И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' из выпадающего списка с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборПравоеЗначение' я выбираю "$$ЧленствоЧПУ$$"								
		...
		*Проверка содержания таблицы
			И таблица 'Список' стала равной по шаблону:
				| 'Период'      | 'Сотрудник'  | 'N' | 'Регистратор'                        | 'Клиент'     | 'Документ оплаты'                            | 'Занятие'                                                                     | 'Документ продажи'        | 'Услуга, сегмент'                                                | 'Сумма' | 'Ассистент' | 'Количество' | 'Сумма без скидки' | 'Структурная единица' |
				| '*.*.* *:*:*' | '$$Тренер$$' | '1' | 'Запись на занятие * от *.*.* *:*:*' | '$$Клиент$$' | 'Членство "$$ЧленствоЧПУ$$" * от *.*.* *:* ' | 'Персональное: *.*.* *:*, $$УслугаПерсональная$$, Фитнес плюс, * (выполнено)' | 'Продажа * от *.*.* *:* ' | '$$УслугаПерсональная$$, $$УслугаПерсональная$$, Розничная цена' | ''      | ''          | '1,000'      | ''                 | 'Фитнес плюс'         |
			И я закрываю все окна клиентского приложения

*Запись на групповое занятие через кнопку "Использовать".				
	*Создание группового занятия для последующей записи.
		Дано я открываю основную форму списка документа "Занятие"
		И я нажимаю на кнопку с именем 'ФормаСоздатьГрупповоеЗанятие'
		Тогда открылось окно "Групповое занятие (создание)"
		...
		И в поле с именем 'Помещение' я ввожу текст "$$Помещение$$"
		И из выпадающего списка с именем 'Помещение' я выбираю "$$Помещение$$"
		И в поле с именем 'ДатаВремяНачала' я ввожу текст "00:00"
		И в поле с именем 'ПродолжительностьЗанятия' я ввожу текст "300"
	...
	*Заходим в номенклатуру групповой услуги и указываем в ней помещение по умолчанию.
		Дано Я открываю навигационную ссылку "$$СсылкаУслугаГрупповая$$"
		И в поле с именем 'УслугаПомещениеПоУмолчанию' я ввожу текст "$$Помещение$$"
		И из выпадающего списка с именем 'УслугаПомещениеПоУмолчанию' я выбираю "$$Помещение$$"
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
	...
	*Запись на групповое занятие.
		...
		И в таблице 'ДоступныеУслугиКлиента' я перехожу к строке:
			| "Группировка предст"  | "Действие предст" | "Количество предст" |
			| "$$УслугаГрупповая$$" | "Использовать"    | "неогр."            |
		И в таблице 'ДоступныеУслугиКлиента' я активизирую поле с именем 'ДоступныеУслугиДействие'	
		И в таблице 'ДоступныеУслугиКлиента' я выбираю текущую строку	
		Тогда открылось окно 'Запись на групповое занятие'			
		И я нажимаю на гиперссылку с именем 'ДекорацияВесьДень'
		И я меняю значение переключателя с именем 'ОтборРежимПоПомещению' на "Основное помещение"			
		И я закрываю все окна клиентского приложения
		Дано Я открываю основную форму обработки "РабочийСтол"
		И в поле с именем 'ТекущийКлиент' я ввожу значение выражения "$$Клиент$$"
		И из выпадающего списка с именем 'ТекущийКлиент' я выбираю точное значение "$$Клиент$$"
	*Проверка формы рецепции на наличие двух занятий.
		И таблица 'ПосещенияНаСегодня' стала равной по шаблону:
			| '1'      | '2'      | '8' | '3'      | '5' | '6'                                                           | '7'  |
			| '00:00 ' | '00:30 ' | ''  | '30 мин' | ''  | ' Посещение по : $$ЧленствоЧПУ$$'                             | '1'  |
			| '00:00 ' | '00:30 ' | ''  | '30 мин' | ''  | '$$УслугаПерсональная$$, $$Тренер$$ Вид Цен: Розничная цена'  | '1 ' |
			| '00:00 ' | '-:-'    | ''  | '5 ч'    | ''  | '$$УслугаГрупповая$$, А. Вид Цен: Розничная цена, записано 1' | '1 ' |
		
Сценарий: Проверка записи клиента в период добавленных дней в членство.//876442
*Удаление переменных и создание новых данных.	
	И я удаляю все переменные
	И я создаю клиента
	И Я создаю Услуга_Персональная
...
*Смена даты активации членства.
	...
	И в таблице "Список" я выбираю текущую строку
	Тогда открылось окно 'Членство "$$ЧленствоЧПУ$$" * от * *:*'		
	И Я запоминаю значение выражения '' в переменную "НачалоПрошлогоМесяца"
	И я выполняю код встроенного языка
		"""bsl
			ДатаНачала = НачалоМесяца(ДобавитьМесяц(ТекущаяДата(), -1));
			Контекст.Вставить("НачалоПрошлогоМесяца", ДатаНачала);
		"""
	И я нажимаю на кнопку с именем 'ДокументЧленствоПакетУслугОперацииАктивировать'
	И в поле с именем 'ИнформацияДатаРучнойАктивации' я ввожу текст "$НачалоПрошлогоМесяца$"
	И Я запоминаю значение выражения 'Формат(ТекущаяДата(), "ДФ=дд")' в переменную "КолВоДней"
	И я нажимаю на кнопку с именем 'ФормаКнопкаПровестиИЗакрыть'

*Повторное открытие членства и добавление дней к сроду действия.
	Тогда открылось окно 'Членства, пакеты услуг (Список)'
	И в таблице "Список" я выбираю текущую строку			
	И я нажимаю на кнопку с именем 'ДобавитьСрокДействия'
	Тогда открылось окно "Изменение условий членства, пакета услуг (создание)"	
	И из выпадающего списка с именем 'Модификатор' я выбираю по строке "Изменение срока действия"
		
	И в поле с именем 'Количество' я ввожу текст "$КолВоДней$"
	И я нажимаю на кнопку с именем '_Дополнительно'
	И в поле с именем 'Дата' я ввожу текст "$НачалоПрошлогоМесяца$"
	И я нажимаю на кнопку с именем 'ФормаКнопкаПровестиИЗакрыть'

*Запись на занятия с рецепции.
	И я закрываю все окна клиентского приложения
	Дано я открываю основную форму обработки "РабочийСтол"		
	...
	И в таблице 'ДоступныеУслугиКлиента' я перехожу к строке:
		| "Группировка предст"     | "Действие предст" | "Количество предст" |
		| "$$УслугаПерсональная$$" | "Использовать"    | "1"                 |
	И в таблице 'ДоступныеУслугиКлиента' я активизирую поле с именем 'ДоступныеУслугиДействие'
	И в таблице 'ДоступныеУслугиКлиента' я выбираю текущую строку
	Тогда открылось окно 'Запись на персональное занятие'
	И я меняю значение переключателя с именем 'ОтборРежимПоСотруднику' на "Любой"
	И в поле с именем 'ОтборСотрудник' я ввожу текст "$$Тренер$$"
	И из выпадающего списка с именем 'ОтборСотрудник' я выбираю "$$Тренер$$"
	И клик по координатам "895" "367"
	Тогда открылось окно '1С:Предприятие'		
	И я нажимаю на кнопку с именем 'Button0'
	*Заполнение персонального занятия.
	Тогда открылось окно 'Персональное занятие (создание) *'			
	И Я запоминаю значение выражения 'Формат(ТекущаяДата()-86400, "ДФ=дд.ММ.гггг")' в переменную "Вчера"
	И в поле с именем 'ДатаЗанятия' я ввожу текст "$Вчера$"
	И в поле с именем 'ВремяНачала' я ввожу текст "23:30"
	И в поле с именем 'Помещение' я ввожу текст "$$Помещение$$"
	И из выпадающего списка с именем 'Помещение' я выбираю "$$Помещение$$"
...
*Создание группового занятия и запись на него.
	И я закрываю все окна клиентского приложения
	Дано я открываю основную форму списка документа "Занятие"			
	И я нажимаю на кнопку с именем 'ФормаСоздатьГрупповоеЗанятие'
	И в поле с именем 'Номенклатура' я ввожу текст "$$УслугаГрупповая$$"
	...
	И я нажимаю на кнопку с именем 'Button0'
	Тогда открылось окно "Групповое занятие (создание) *"
	И в поле с именем 'ДатаВремяНачала' я ввожу текст "22:00"
	И в поле с именем 'Сотрудник' я ввожу текст "$$Тренер$$"
	И из выпадающего списка с именем 'Сотрудник' я выбираю "$$Тренер$$"
	И в поле с именем 'Помещение' я ввожу текст "$$Помещение$$"
	И из выпадающего списка с именем 'Помещение' я выбираю "$$Помещение$$"
	Когда открылось окно "Групповое занятие (создание) *"
	И я нажимаю на кнопку с именем 'КнопкаСоставЗанятия'
	И в поле с именем 'ПолеКлиента_0' я ввожу текст "$$Клиент$$"
	И из выпадающего списка с именем 'ПолеКлиента_0' я выбираю "$$Клиент$$"
	И элемент формы с именем 'ПолеОснованияОплаты_0' стал равен по шаблону 'Членство: \"$$ЧленствоЧПУ$$\" от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}'
	И я нажимаю на кнопку с именем 'КнопкаСтатусаПрибытия_0'
	И я меняю значение переключателя с именем 'СтатусЗанятия' на "Выполнено"
	И я нажимаю на кнопку с именем 'ФормаКнопкаПровестиИЗакрыть'
				
*Проверка записей на занятие с рецепции.
	И я закрываю все окна клиентского приложения
	Дано я открываю основную форму обработки "РабочийСтол"		
	И в поле с именем 'ТекущийКлиент' я ввожу текст "$$Клиент$$"
	И из выпадающего списка с именем 'ТекущийКлиент' я выбираю "$$Клиент$$"
	И таблица 'ПосещенияНаСегодня' стала равной:
		| '1'      | '2'      | '3'      | '5' | '6'                                                                       | '7'  | '8' |
		| '23:30 ' | '00:00 ' | '30 мин' | ''  | '$$УслугаПерсональная$$, $$Тренер$$ Вид Цен: Розничная цена'              | '1 ' | ''  |
		| '22:00 ' | '22:30 ' | '30 мин' | ''  | '$$УслугаГрупповая$$, $$Тренер$$ Вид Цен: Розничная цена, прибыло 1 из 1' | '1 ' | ''  |
...
Сценарий: Возврат настроек.
	...
	И в таблице 'ДеревоОбщихНастроек' я перехожу к строке:
		| "Настройка"                                     |
		| "Разрешить заморозку меньше минимального срока" |
	И в таблице 'ДеревоОбщихНастроек' я снимаю флаг с именем 'ДеревоОбщихНастроекЗначение'
	И в таблице 'ДеревоОбщихНастроек' я завершаю редактирование строки
	И В текущем окне я нажимаю кнопку командного интерфейса "Основное"
	И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'
...
//Настраиваем время на "Рабочие дни".
	...
	И Я запоминаю значение выражения 'Формат(ДобавитьМесяц(ТекущаяДата(), 1)-84600, "ДФ=дд.ММ.гггг")' в переменную "ДатаОкончанияЧленства"
	И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
...
//Проверка доступности членства на рецепции.
	...
	И таблица 'ДоступныеУслугиКлиента' содержит строки по шаблону:
		...
		| 'Членства'           | ''                                                               |
		| '$$ЧленствоЧПУ$$'    | 'Недоступен по времени. Дата окончания: $ДатаОкончанияЧленства$' |
...
//Пересоздания клиентов продолжение
	Дано Я открываю навигационную ссылку "$$СсылкаЧленствоЧПУ$$"
