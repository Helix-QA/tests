...
Сценарий: 01. Создание клиента
	*Процедура 1. Переменные; Объявление переменных
	...
	*Процедура 2. Заполнение карточки клиента; Заполнение информации
	...
	*Создаю задачи
		...
		Тогда элемент формы с именем 'ИсторияВзаимодействия' стал равен по шаблону
			|'    	    		<div class=\"text-message\"><input style=\"margin:0 2px 0 0; width:12px; height:12px;\" title=\"Отметить как выполнено\" type=\"checkbox\" class=\"executetask\" name=\"Взаимодействие\" id=\"*"><span class=\"interaction\" style=\"color: #000\"> Без темы. </span></div>'|
	...
	*Проверка Мед. обследование (Печать)
		И я нажимаю на кнопку с именем 'Печать'
		И я закрываю текущее окно			
	
	*Проверка Параметры тела (Печать)
		И я нажимаю на кнопку с именем 'Печать'
		И я закрываю текущее окно	
	
	*Проверка Индивидуальные достижения (Печать)
		И я нажимаю на кнопку с именем 'Печать'
		И я закрываю текущее окно	

	*Перехожу в окно E-mail					
...
Сценарий: 02. Статус клиента
	...
	*Проверка статуса "Клиент клуба"; Продажа услуги
		...
		*Проверка статуса "Член клуба"; Продажа членства
		И я закрываю все окна клиентского приложения
		И В командном интерфейсе я выбираю "Главное" "Рецепция"
		И в поле с именем 'ТекущийКлиент' я ввожу значение выражения "$$Клиент$$"
	...
	*Проверка статуса "Бывший член клуба"; Возврат членства; Установка N дней до бывшего клиента.
