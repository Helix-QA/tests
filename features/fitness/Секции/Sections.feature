...
Сценарий: Проверка автопродления и скидки на новый пакет услуг.
	...
	И из выпадающего списка с именем 'ЭквайринговыйТерминал' я выбираю точное значение "02.Юкасса, Основная ораганизация (эквайринг)"
...
Сценарий: Проверка нового функционала выписки из состава секционных занятий по тикету - 813327.
	*Создание данных.
		И я удаляю все переменные
		И я удаляю объекты "Справочники.ГруппыКлиентов" без контроля ссылок
		И я удаляю объекты "Справочники.Циклы" без контроля ссылок
	...
	*Определение кол-ва занятий по дням.
		И Я запоминаю значение выражения '' в переменную "КоличествоПонедельников"
		И Я запоминаю значение выражения '' в переменную "КоличествоСред"
		И Я запоминаю значение выражения '' в переменную "КоличествоПятниц"	
		И я выполняю код встроенного языка
			"""bsl
				// Определяем начальную и конечную даты периода
				ТекущаяДата = ТекущаяДата(); // Например, 06.05.2025
				...
				// Записываем результат в переменную контекста
				Контекст.НужноеКолВоДней = ОбщееКоличество;
				Контекст.КоличествоПонедельников = КоличествоПонедельников;
				Контекст.КоличествоСред = КоличествоСред;
				Контекст.КоличествоПятниц = КоличествоПятниц;
			"""

	*Продажа секционного пакета услуг.
		Дано я открываю основную форму документа "Реализация"
		И из выпадающего списка с именем 'Контрагент' я выбираю по строке "$$Клиент$$"
		И я нажимаю на кнопку с именем 'ЗапасыКнопкаДобавить'
		И в таблице 'Запасы' я выбираю текущую строку
	...
	*Открытие карточки проданного пакета услуг.
		Дано Я открываю навигационную ссылку "$$СсылкаКлиента$$"
		И В текущем окне я нажимаю кнопку командного интерфейса "Пакеты"
		И в таблице 'Список' я перехожу к строке:
			| 'По порядку' | 'Номенклатура'     |
			| '1'          | '$$ПакетСекции1$$' |
		И в таблице 'Список' я выбираю текущую строку		
		И я нажимаю на кнопку с именем 'ДниПосещенияУдалить_*'					
	*Сохранение идентификаторов по дням и проверка новой всплывающей таблички с "Дни посещений".
		И Я запоминаю значение выражения '' в переменную "Идентификаторы"
		И я запускаю мониторинг каталога "$КаталогПроекта$\features\fitness\Секции" для внешних событий(Расширение)
				...
				"""bsl
					...
					Для Каждого ОкноПриложения Из СписокОконПриложения Цикл
						...
						Если ОкноПриложения.Содержимое.Количество() > 0
							И ОкноПриложения.Содержимое[0].ИмяФормы = "Документ.ЧленствоПакетУслуг.Форма.ФормаЭлементаТакси" Тогда
...
//			"""
		...
		И Я запоминаю значение выражения '' в переменную "ИД"
		И для каждого значения "ИД" из массива "$Идентификаторы$"
			Если элемент формы с именем 'ПредставлениеДня_$ИД$' стал равен "Пн. 22:00 $$УслугаГрупповая$$ - $$Тренер$$" Тогда
				И Я запоминаю значение выражения '$ИД$' в переменную "Понедельник"
				Тогда я продолжаю цикл	
			Если элемент формы с именем 'ПредставлениеДня_$ИД$' стал равен "Ср. 22:00 $$УслугаГрупповая$$ - $$Тренер$$" Тогда
				И Я запоминаю значение выражения '$ИД$' в переменную "Среда"
				Тогда я продолжаю цикл
			Если элемент формы с именем 'ПредставлениеДня_$ИД$' стал равен "Пт. 22:00 $$УслугаГрупповая$$ - $$Тренер$$" Тогда
				И Я запоминаю значение выражения '$ИД$' в переменную "Пятница"
				Тогда я продолжаю цикл
	
	*Проверка выписки из занятий и заполнение регистра "Очередь записи на занятия".
		И элемент формы с именем 'ЗаполнитьВсеДниИзСекции' не доступен
		И я нажимаю на кнопку с именем 'ДниПосещенияУдалить_$Пятница$'
		И элемент формы с именем 'ЗаполнитьВсеДниИзСекции' доступен
		И я нажимаю на кнопку с именем 'СохранитьДниПосещений'
	
	*Открытие регистра и проверка заполнения.
		Дано я открываю основную форму списка регистра сведений "ОчередьЗаписиНаЗанятия"
		И Я запоминаю значение выражения '' в переменную "Текущая"
		И Я определяю даты выписки/записи из/на занятия.
		*Установка фильтра в списке.			
			...
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' в поле с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборЛевоеЗначение' я ввожу текст "Занятие.Дата.Части дат.День недели"
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' из выпадающего списка с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборЛевоеЗначение' я выбираю по строке "Занятие.Дата.Части дат.День недели"
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' я завершаю редактирование строки
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' я активизирую поле с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборПравоеЗначение'
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' я выбираю текущую строку
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' в поле с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборПравоеЗначение' я ввожу текст "5"
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' я завершаю редактирование строки
			И я нажимаю на кнопку с именем 'ФормаЗакончитьРедактирование'
		*Проверка содержания регистра.
			И Я запоминаю значение выражения '' в переменную "ДатаВыписки"		
			И для каждого значения "ДатаВыписки" из массива "$Текущая$"
				И таблица 'Список' содержит строки по шаблону:
					| 'Основание'                                                                         | 'Занятие'                                                                             | 'Выписать из занятия' | 'Ошибка записи' |
					| 'Пакет услуг "$$ПакетСекции1$$" * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:* ' | 'Групповое: $ДатаВыписки$ 22:00, $$УслугаГрупповая$$, Фитнес плюс, * (запланировано)' | 'Да'                  | ''              |
			И я выполняю код встроенного языка на сервере
				"""bsl
					РегламентныеЗаданияСерверФитнес.КонтрольВыполненияЗанятий();
				"""

	*Проверка списка групповых занятий на наличие клиента в них.
		*Настройка отбора по секции.
			Дано я открываю основную форму списка документа "Занятие"
			И я нажимаю на кнопку с именем 'НастройкаСписка'
			Тогда открылось окно "Настройка списка"
			И я нажимаю на кнопку с именем 'ФормаСтандартныеНастройки'
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' я нажимаю на кнопку с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборДобавитьЭлементОтбора'
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' из выпадающего списка с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборЛевоеЗначение' я выбираю по строке "Группа клиентов"
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' я активизирую поле с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборПравоеЗначение'
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' в поле с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборПравоеЗначение' я ввожу текст "$$Секция1$$"
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' из выпадающего списка с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0ОтборПравоеЗначение' я выбираю "$$Секция1$$"
			И в таблице 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент0Отбор' я завершаю редактирование строки
			И я нажимаю на кнопку с именем 'ФормаЗакончитьРедактирование'
			Когда открылось окно "Занятия (список)"
			И из выпадающего списка с именем 'ОтборКонтрагент' я выбираю "$$Клиент$$"
		*Проверка занятий на которые не записан клиент.
			И Я запоминаю значение выражения '' в переменную "ДатаВыписки"		
			И для каждого значения "ДатаВыписки" из массива "$Текущая$"				
				И таблица "Список" не содержит строки по шаблону:
					| 'Дата'          | 'Тип занятия'       | 'Автор' | 'Начало' | 'Структурная единица' | 'Окончание' | 'Номер' | 'Статус занятия' | 'Услуга'              | 'Исполнитель' | 'Комментарий' |
					| '$ДатаВыписки$' | 'Групповое занятие' | ''      | '22:00'  | 'Фитнес плюс'         | '22:30'     | '*'     | 'Запланировано'  | '$$УслугаГрупповая$$' | '$$Тренер$$'  | ''            |

	*Проверка записи клиента обратно на секционные занятия.
		Дано Я открываю навигационную ссылку "$$СсылкаКлиента$$"
		И В текущем окне я нажимаю кнопку командного интерфейса "Пакеты"
		И в таблице 'Список' я перехожу к строке:
			| 'По порядку' | 'Номенклатура'     |
		...
		И я нажимаю на кнопку с именем 'ДниПосещенияУдалить_*'
		И элемент формы с именем 'ЗаполнитьВсеДниИзСекции' доступен					
		И я нажимаю на кнопку с именем 'ЗаполнитьВсеДниИзСекции'
		И элемент формы с именем 'ЗаполнитьВсеДниИзСекции' не доступен		
		И я нажимаю на кнопку с именем 'СохранитьДниПосещений'
		И я закрываю все окна клиентского приложения
		...
		*Проверка содержания регистра.
			...
			И для каждого значения "ДатаВыписки" из массива "$Текущая$"
				И таблица 'Список' содержит строки по шаблону:
					| 'Основание'                                                                         | 'Занятие'                                                                             | 'Выписать из занятия' | 'Ошибка записи' |
					| 'Пакет услуг "$$ПакетСекции1$$" * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} *:* ' | 'Групповое: $ДатаВыписки$ 22:00, $$УслугаГрупповая$$, Фитнес плюс, * (запланировано)' | 'Нет'                 | ''              |
			И я выполняю код встроенного языка на сервере
				"""bsl
					РегламентныеЗаданияСерверФитнес.КонтрольВыполненияЗанятий();
				"""
		*Проверка списка групповых занятий на наличие клиента в них.
				*Настройка отбора по секции.
				...
				Когда открылось окно "Занятия (список)"
				И из выпадающего списка с именем 'ОтборКонтрагент' я выбираю "$$Клиент$$"
			*Проверка занятий на которые записало клиента обратно.
				И Я запоминаю значение выражения '' в переменную "ДатаВыписки"		
				И для каждого значения "ДатаВыписки" из массива "$Текущая$"				
					И таблица "Список" не содержит строки по шаблону:
						| 'Дата'          | 'Тип занятия'       | 'Автор' | 'Начало' | 'Структурная единица' | 'Окончание' | 'Номер' | 'Статус занятия' | 'Услуга'              | 'Исполнитель' | 'Комментарий' |
