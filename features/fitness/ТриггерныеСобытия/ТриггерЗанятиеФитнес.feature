...
//	Дано я открываю внешнюю обработку или отчет "D:\Репозиторий(HELIX)\tests\epf\Test_Api.epf" (Расширение)
	...
	И в поле с именем "appointment" я ввожу текст "$НомерЗанятия$"
	И Я запоминаю значение выражения 'Формат(Дата("00010101" + СтрЗаменить($ЧасСверху$, ":", "") + "00"), "ДФ=Ч:мм")' в переменную "ЧасСверху"
	И из выпадающего списка с именем 'appointment' я выбираю '$НомерЗанятия$ ({Формат(ТекущаяДата(),"ДФ=дд.ММ.гггг")} $ЧасСверху$:00)'
//	И я нажимаю кнопку выбора у поля с именем 'appointment'
//		Тогда открылось окно "Выбор типа данных"
//		И в таблице '' я перехожу к строке:
//			| ""        |
...
//	И в таблице 'Список' я перехожу к строке по шаблону:
//		| "Начало"                     | "Тип занятия"       | "Услуга"                |
//		| "*.*.* $ЧасСверхуДляApi$:00" | "Групповое занятие" | "*$$УслугаГрупповая$$*" |
