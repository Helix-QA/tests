...
//	Дано я открываю внешнюю обработку или отчет "D:\Репозиторий(HELIX)\tests\epf\Test_Api.epf" (Расширение)
...
//Активация триггера - Перенос даты и времени начала.
	И я закрываю все окна клиентского приложения
	И Я запоминаю значение выражения 'ТекущаяДата()+14200' в переменную "ЧасСверху"
	И Я запоминаю значение выражения 'Формат($ЧасСверху$, "ДФ=dd.MM.yyyy")' в переменную "ДеньДатой"
	И Я запоминаю значение выражения 'Формат($ЧасСверху$, "ДФ=ЧЧ:мм")' в переменную "ВремяВЧасах"										
	Дано Я открываю основную форму списка документа "Занятие"
	И я нажимаю на кнопку с именем 'ФормаСоздатьГрупповоеЗанятие'
	И в поле с именем 'Дата' я ввожу текст "$ДеньДатой$"	
	И в поле с именем 'ДатаВремяНачала' я ввожу значение выражения "$ВремяВЧасах$"
	И в поле с именем 'Номенклатура' я ввожу текст "$$УслугаГрупповая$$"
	И из выпадающего списка с именем 'Номенклатура' я выбираю по строке "$$УслугаГрупповая$$"
	И я нажимаю на кнопку с именем 'Button0'
	И в поле с именем 'Помещение' я ввожу текст "$$Помещение$$"
...
//Смена времени
	Дано Я открываю навигационную ссылку "$ГрупповоеЗанятие$"
	И я нажимаю на кнопку с именем 'КнопкаНастройкиЗанятия'
	И Я запоминаю значение выражения 'ТекущаяДата()+84600' в переменную "ДеньСверху"
	И Я запоминаю значение выражения 'Формат($ДеньСверху$, "ДФ=dd.MM.yyyy")' в переменную "ДеньДатой"
	И Я запоминаю значение выражения 'Формат($ДеньСверху$, "ДФ=ЧЧ:мм")' в переменную "ВремяВЧасах"
	И в поле с именем 'Дата' я ввожу текст "$ДеньДатой$"
	И в поле с именем 'ДатаВремяНачала' я ввожу значение выражения "$ВремяВЧасах$"			
	И я нажимаю на кнопку с именем 'ФормаКнопкаПровестиИЗакрыть'
...
//Активация триггера- Онлайн бронирование без оплаты
	...
	И я нажимаю на кнопку с именем 'Оплатить'
	И Я закрываю окно "Исходные данные чека"
//Старые переменные, не учел сдвиг даты/дня.
...
//	Дано Я запоминаю значение выражения 'Формат(ТекущаяДата()+14404, "ДФ=ЧЧ:мм")' в переменную "ДваЧасаСверху"
//	И Я запоминаю значение выражения 'Формат(ТекущаяДата()+7202, "ДФ=dd.MM.yyyy" "ДФ=ЧЧ:мм")' в переменную "ЧасСверху"
//	И Я запоминаю значение выражения 'Формат(ТекущаяДата()+7202, "ДФ=dd.MM.yyyy" "ДФ=Ч:мм")' в переменную "ЧасСверхуДляApi"
	И Я запоминаю значение выражения 'ТекущаяДата()+14404' в переменную "ДваЧасаСверху"
...
//Добавление настройки с требованием основания для записи на занятие.
	...
	И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'		
//Разбиваем ранеее полученную переменную на дату и время.	
	И Я запоминаю значение выражения 'Формат($ДваЧасаСверху$, "ДФ=dd.MM.yyyy")' в переменную "ДеньДатой"
	И Я запоминаю значение выражения 'Формат($ДваЧасаСверху$, "ДФ=ЧЧ:мм")' в переменную "ВремяВЧасах"
//Создаю новое занятие.
	Дано Я открываю основную форму списка документа "Занятие"
	И я нажимаю на кнопку с именем 'ФормаСоздатьГрупповоеЗанятие'
	И в поле с именем 'Дата' я ввожу текст "$ДеньДатой$"
	...
	И я нажимаю на кнопку с именем 'ФормаКнопкаПровестиИЗакрыть'
	И Я запоминаю значение выражения '' в переменную "НомерЗанятия"
//Из заголовка занятия вытягиванием его номер, для более удобного поиска нужного занятия.
	И я выполняю код встроенного языка
		"""bsl
			// Разделяем строку по запятым (с учетом пробелов после запятых)
			МассивЧастей = СтрРазделить(Контекст.ЗаголовокФормы, ",");

			// Берем последний элемент массива
			ПоследняяЧасть = МассивЧастей[МассивЧастей.Количество() - 1];

			// Обрезаем пробелы слева и справа
			ПоследняяЧасть = СокрЛП(ПоследняяЧасть);

			// Находим позицию открывающей скобки
			ПозицияСкобки = СтрНайти(ПоследняяЧасть, "(");

			// Если скобка найдена, извлекаем число до скобки
			Если ПозицияСкобки > 0 Тогда
				НомерЗанятия = СокрЛП(Сред(ПоследняяЧасть, 1, ПозицияСкобки - 1));
			Иначе
				НомерЗанятия = ПоследняяЧасть;
			КонецЕсли;

			// Теперь НомерЗанятия содержит "186"
			// Можно записать в другую переменную, например:
			Контекст.НомерЗанятия = НомерЗанятия;
		"""
	И я запоминаю значение выражения 'Формат(Число($НомерЗанятия$), "ЧЦ=11; ЧВН=; ЧГ=0")' в переменную "НомерЗанятия"
...
//	Дано я открываю внешнюю обработку или отчет "D:\Репозиторий(HELIX)\tests\epf\Test_Api.epf" (Расширение)
	...
	И я активизирую поле с именем 'appointment'
	И я нажимаю кнопку выбора у поля с именем 'appointment'
	И я активизирую дополнение формы с именем 'СтрокаПоиска'
	И в дополнение формы с именем 'СтрокаПоиска' я ввожу текст "$НомерЗанятия$"
