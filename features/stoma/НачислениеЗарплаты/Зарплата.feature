...
*Создание нового начисления зарплаты.
	...
	И таблица 'Затраты' содержит строки по шаблону:
		| 'N' | 'Дата'                                     | 'Документ'                                                  | 'Персонал'      | 'Вид начисления'                                                                        | 'Пациент'      | 'Номенклатура' | 'План' | 'Факт' | 'База'   | 'Размер (%, сумма)' | 'Сумма'   |
		| '*' | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}' | 'Реализация * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} ' | '$$Сотрудник$$' | 'Процент от оказанных оформленных сотрудником услуг, за каждый час сверх менее 0 руб. ' | '$$Пациент$$ ' | '$$Услуга$$'   | ''     | ''     | '515,00' | '100,00'            | '515,00'  |
		| '*' | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}' | 'Реализация * от {Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} ' | '$$Сотрудник$$' | 'Процент от оказанных оформленных сотрудником услуг, за каждый час сверх менее 0 руб. ' | '$$Пациент$$ ' | '$$Услуга$$'   | ''     | ''     | '257,50' | '100,00'            | '257,50'  |
		| '*' | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")}' | 'Приём * окончен в *:*, * * *. '                            | '$$Сотрудник$$' | 'Процент от оказанных оформленных сотрудником услуг, за каждый час сверх менее 0 руб. ' | '$$Пациент$$ ' | '$$Услуга$$'   | ''     | ''     | '257,50' | '100,00'            | '257,50'  |
