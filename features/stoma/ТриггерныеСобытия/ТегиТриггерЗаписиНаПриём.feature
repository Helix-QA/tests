...
Сценарий: Создание триггера - Создание записи на прием.
	...
	*Создание триггера - Создание записи на прием.
		...
		*Вид действия - "Поставить задачу", исполнитель - "Сотрудник".
		...
		*Вид действия - "Поставить задачу", исполнитель - "Автор предварительной записи".875996
			И из выпадающего списка с именем 'ВидДействия_Индекс_1' я выбираю точное значение "Поставить задачу"
			И я перехожу к закладке с именем 'ВсплывающаяГруппаПараметровДействия_Индекс_1'
			И из выпадающего списка с именем 'ВидИсполнителя_Индекс_1' я выбираю точное значение "Автор предварительной записи"
			И в поле с именем 'ШаблонСообщения_Индекс_1' я ввожу текст "$$Шаблон$$"
			И из выпадающего списка с именем 'ШаблонСообщения_Индекс_1' я выбираю по строке "$$Шаблон$$"
			И я нажимаю на кнопку с именем 'ВзаимодействиеРежимНапоминания_Индекс_1'
            И в меню формы я выбираю 'не напоминать'
	...
	*Проверка журнала регистрации триггерных событий.
		...
		*Переход в журнал регистрации и сброс даты отбора	
			...
			И таблица 'Список' содержит строки по шаблону:
				...
				| '$Наименование$'     | 'Поставить задачу сотруднику' | 'Зарегистрировано' | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} в *:*:*' | '$$Пациент$$ ' | '$$Шаблон$$ (не выполнена)' | ''                 | 'Прием не подтвержден, *:* - *:*, * * *. ' | ''                        |
				| '$Наименование$'     | 'Поставить задачу'            | 'Зарегистрировано' | '{Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг")} в *:*:*' | '$$Пациент$$ ' | '$$Шаблон$$ (не выполнена)' | ''                 | 'Прием не подтвержден, *:* - *:*, * * *. ' | ''                        |
