...
Сценарий: Проверка статуса звонка от клиента
	...
	*Проверка страницы со звонком
		Если открылось окно "Входящий звонок: *" Тогда
			И элемент формы с именем 'ДекорацияКартинкаЗвонок' стал равен "Декорация картинка звонок"
			И элемент формы с именем 'ДекорацияНадписьДатаВремяЗвонка' стал равен по шаблону "$$ЧисловаДата$$ - *"
	...
	*Проверка спр. история вызовов
		...
		И в таблице 'Список' я перехожу к строке по шаблону:
			| 'Дата звонка'   | 'Тип звонка' | 'Телефон клиники' | 'Телефон пациента'   | 'Сотрудник' | 'Статус'   |
			| '$$ТекущДата$$' | 'Входящий'   | 'admin, 300'      | '$$ТелефонКлиента$$' | 'Админ'     | 'Разговор' |
	...
	*Проверка в спр. "История вызовов"
		И я перехожу по навигационной ссылке "e1cib/command/РегистрСведений.ИсторияВызовов.Команда.ИсторияЗвонков"
		И в таблице 'Список' я перехожу к строке по шаблону:
			| 'Дата звонка'   | 'Тип звонка' | 'Статус пациента' | 'Телефон клиники' | 'Результат звонка'           | 'Телефон пациента'   | 'Пациент'                          | 'Статус'   |
			| '$$ТекущДата$$' | 'Входящий'   | 'Отказ'           | 'admin, 300'      | 'Звонок не связан с приемом' | '$$ТелефонКлиента$$' | '$$Фамилия$$ $$Имя$$ $$Отчество$$' | 'Успешный' |
		И в таблице 'Список' я выбираю текущую строку
	...
	*Проверка звонка в клиент 2ой кнопки
		...
		И элемент формы с именем 'НачалоВызова_0' стал равен по шаблону "* \| $$ПолнаяДата$$"
		И элемент формы с именем 'НомерТелефона_0' стал равен "$$ТелефонКлиента$$"
		И элемент формы с именем 'Статус_' стал равен "Пропущен"
		И элемент формы с именем 'Статус_0' стал равен "Успешный"	
...
Сценарий: Проверка пропущенного
	...
	*Проверка в истории вызовов
		...
		И в таблице 'Список' я перехожу к строке по шаблону:
			| 'Дата звонка'   | 'Тип звонка' | 'Телефон клиники' | 'Телефон пациента'    | 'Сотрудник' | 'Статус'   |
			| '$$ТекущДата$$' | 'Входящий'   | 'admin, 300'      | '$$ТелефонКлиента2$$' | 'Админ'     | 'Пропущен' |
		И в таблице 'Список' я выбираю текущую строку
