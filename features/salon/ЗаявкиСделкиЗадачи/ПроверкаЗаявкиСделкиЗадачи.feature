...
Сценарий: Проверяю заявки
	...
	*Кнока "Заметка"
		...
		И в поле с именем 'ТекстЗаметки' я ввожу текст "Проверка"
		И я нажимаю на кнопку с именем 'КнопкаСохранитьИзменения'

	*Проверка на заполнение задачи и заметки
		...
		Тогда элемент формы с именем 'ИсторияВзаимодействия' стал равен по шаблону
				|'           		<div class=\"text-message\"><span style=\"color: #000\">Проверка</span></div>  '|
...
Сценарий: Проверка кнопки "Перейти к диалогу" - 1
		И В командном интерфейсе я выбираю "CRM" "Сделки"
		И я нажимаю на кнопку с именем 'К_ОткрытьСписок'
		И в таблице 'Список' я перехожу к строке:
			| 'Сотрудник'   |
...
Сценарий: Проверка кнопки "Перейти к диалогу" - 2
		И В командном интерфейсе я выбираю "CRM" "Заявки"
		И я нажимаю на кнопку с именем 'ОткрытьСписок'
		И в таблице 'Список' я перехожу к строке:
			| 'ФИО'                              | 'Кем создано'                   |
			| '$$Фамилия$$ $$Имя$$ $$Отчество$$' | 'Администратор салона красоты ' |
		И в таблице 'Список' я выбираю текущую строку
		И я нажимаю на кнопку с именем 'СоздатьСообщение'
		И я нажимаю на кнопку с именем 'КнопкаПерейтиВЧат'
							
Сценарий: Проверка кнопки "Перейти к диалогу" - 3
		И В командном интерфейсе я выбираю "Продажи" "Клиенты"
		И в поле с именем 'РасширенныйПоиск' я ввожу текст "$$Фамилия$$ $$Имя$$ $$Отчество$$"
