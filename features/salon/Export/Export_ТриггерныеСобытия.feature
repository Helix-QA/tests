...
Сценарий: Я создаю мастера салона красоты для триггерных событий
	...
	И Я записываю идентификаторы КИ в комментарий.
	И я жду, что поле "Комментарий" перестанет быть пустым в течении 30 секунд
	И я запоминаю значение поля с именем 'Комментарий' как 'Идентификаторы'
	...
	И для каждого значения "ИД" из массива "$Идентификаторы$"
		Если элемент формы с именем 'ВидКИ_КонтактнаяИнформацияПоле$ИД$' стал равен 'Телефон' Тогда
			И в поле с именем 'КонтактнаяИнформацияПоле$ИД$' я ввожу текст '$НомерТелефона$'
		Если элемент формы с именем 'ВидКИ_КонтактнаяИнформацияПоле$ИД$' стал равен 'Адрес' Тогда
			Тогда я продолжаю цикл
		Если элемент формы с именем 'ВидКИ_КонтактнаяИнформацияПоле$ИД$' стал равен 'Email' Тогда
			И Я запоминаю случайное число в переменную "Email"
			И я запоминаю значение выражения 'Формат($Email$, "ЧГ=0")' в переменную "$$Email$$"
			И в поле с именем 'КонтактнаяИнформацияПоле$ИД$' я ввожу текст 'dimitrii$$Email$$@gmail.com'
	И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'


Сценарий: Я записываю идентификаторы КИ в комментарий.
	И Я запоминаю значение выражения '' в переменную "Идентификаторы"
	И я запускаю мониторинг каталога "$КаталогПроекта$\features\salon\ТриггерныеСобытия" для внешних событий(Расширение)
		...
		"""bsl
			...
			Для Каждого ОкноПриложения Из СписокОконПриложения Цикл
				...
				Если ОкноПриложения.Содержимое.Количество() > 0
					И ОкноПриложения.Содержимое[0].ИмяФормы = "Справочник.Сотрудники.Форма.ФормаЭлемента" Тогда
			
					ОсновнаяГруппа = ОкноПриложения.Содержимое[0].Элементы.Найти("ГруппаПолейВводаКонтактнойИнформации");

					Если ОсновнаяГруппа <> Неопределено Тогда

						Для Каждого Подгруппа Из ОсновнаяГруппа.ПодчиненныеЭлементы Цикл
							Идентификаторы = Идентификаторы + ?(ПустаяСтрока(Идентификаторы), "", ";") + СтрЗаменить(Подгруппа.ПодчиненныеЭлементы[2].Имя, "КонтактнаяИнформацияПоле", "");
						КонецЦикла;
...
Сценарий: Я создаю сервис рассылки для триггерного события по тикету - 858203.
	И я удаляю объекты "Справочники.СервисыРассылки" без контроля ссылок	
	И я открываю основную форму справочника "СервисыРассылки"
	Тогда открылось окно 'Сервис SMS-рассылки (создание)'	
	И из выпадающего списка с именем 'SMSСервис' я выбираю точное значение "Helix SMS"
	И в поле с именем 'Пользователь' я ввожу текст "123"
	И в поле с именем 'Пароль' я ввожу текст "123"
	И в поле с именем 'Подпись' я ввожу текст "123"
	И я нажимаю на кнопку с именем 'ФормаКнопкаСохранитьИЗакрыть'
